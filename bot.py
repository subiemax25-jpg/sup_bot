...
async def moderate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        # ── Автоначисление очков (барахолка — без очков; правится /addpoints и /fixname) ──
        points_line = ""
        user_bonus  = ""
        if ptype in ("announce", "review"):
            pts        = 2 if ptype == "review" else 1
...
async def _post_init(app: Application):
    ...
    if ADMIN_ID:
        admin_cmds = user_cmds + [
            ...
            BotCommand("fixname",        "Исправить подпись в рейтинге"),
            BotCommand("window",         "Позвать на воду (окно по Windy)"),
